...
const RentableTransactionsTable = () => {
      ...
      <Table>
        <thead>
          <tr>
            ...
            <TableHeader onClick={() => handleSort("status")}>SALDO DISPONÍVEL</TableHeader>
          </tr>
        </thead>
        <tbody>
          {currentData.map((contrato, index) => (
            <TableRow key={index}>
              ...
              <TableCell>{helpers.formatToBrazilianCurrency(contrato.currentIncome)}</TableCell>
              <TableCell>{helpers.formatToBrazilianCurrency(contrato.currentIncome - contrato.amountWithdrawn)}</TableCell>
            </TableRow>
          ))}
        </tbody>
      </Table>
...
export default RentableTransactionsTable;